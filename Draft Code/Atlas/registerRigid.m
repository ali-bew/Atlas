function [outputArg1] = registerRigid(inputArg1)
...
% to run this code place the dataset in the same directory of code.
    ...
    for i = 7:7 %i is the fixed image, the loop is for testing all atlases choices
        ...
        for j = 0:N
            ...
            if i ~= j
                outputFolder = strcat(strcat(currentFolder ,'\Data\resultsRigid\output'),num2str(j));              
                ...
                if isfile(fixedImagePath) && isfile(movingImagePath)
                    mkdir(outputFolder);
                    command = ['elastix -f "' fixedImagePath '" -m "' movingImagePath '" -out "' outputFolder '" -p parameters1-0000.txt ' ' -fMask "' fixedMaskPath '"' ' -mMask "' movingMaskPath '"'];
                    disp(command)
                    [status,cmdout] = dos(command);

                    cmdout

                    labelFolder = strcat(strcat(currentFolder ,'\Data\resultsRigid\label'),num2str(j));
                    ...
                    command2 = ['transformix -in "' labelPath '" -out "' labelFolder '" -tp "' transformPath0 '"'];
                ...
                fixedImagePath = strcat(currentFolder , '\Data\Training_Set\IBSR_',ppi,num2str(i),'\IBSR_',ppi,num2str(i),'.nii');
                mkdir(outputFolder);
                labelFolder = strcat(strcat(currentFolder ,'\Data\resultsRigid\label'),num2str(i));
